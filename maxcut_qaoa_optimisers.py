...
from qiskit.algorithms.optimizers import COBYLA, NELDER_MEAD, L_BFGS_B, SLSQP
...
def main(track_mlflow=False):
    if track_mlflow:
        ...
        experiment_name = "QAOA-classical-opts"
    ...
    N = instance_size
    print(f"Running job on instance size of N={instance_size} for various classical optimizers")
    OPTIMIZERS = [
        NELDER_MEAD(disp=True, adaptive=True, tol=0.1, maxfev=MAX_ITERATIONS),
        COBYLA(maxiter=MAX_ITERATIONS),
        L_BFGS_B(maxfun=MAX_ITERATIONS),
        SLSQP(maxiter=MAX_ITERATIONS)
    ]
    ...
    for i, graph_instance in enumerate(G_instances):
        with mlflow.start_run():
            ...
            mlflow.log_param("Source", instance_type_logging)
            graph_features = {
                f"{str(key)}_size_{instance_size}": val
            ...
            for optimizer in OPTIMIZERS:
                quant_alg = "QAOA"
                print(
                    f"\n{'-'*10} Simulating Instance on Quantum using {quant_alg} and {optimizer._method} {'-'*10}\n"
                )
                ...
                def store_intermediate_result(eval_count, parameters, mean, std):
                    if track_mlflow:
                        mlflow.log_metric(
                            f"energy_{quant_alg}_{instance_size}_{optimizer._method}",
                            mean,
                            step=len(counts),
                        )
                        mlflow.log_metric(
                            f"min_energy_{quant_alg}_{instance_size}__{optimizer._method}",
                ...
                if track_mlflow:
                    ...
                    mlflow.log_metric(
                        f"energy_gap_{quant_alg}_instance_size_{instance_size}_optimizer_{optimizer._method}",
                        energy_gap,
                    )
                    mlflow.log_metric(
                        f"final_energy_{quant_alg}_instance_size_{instance_size}_optimizer_{optimizer._method}",
                        algo_result.eigenvalue.real,
                    )
                    mlflow.log_metric(
                        f"maxcut_objective_{quant_alg}_instance_size_{instance_size}_optimizer_{optimizer._method}",
                        algo_result.eigenvalue.real + offset,
                    )
                    mlflow.log_metric(
                        f"solution_objective_{quant_alg}_instance_size_{instance_size}_optimizer_{optimizer._method}",
                    ...
                    with make_temp_directory() as temp_dir:
                        # Plot Network Graph
                        pylab.clf()
                        graph_plot_fn = f"network_plot_{quant_alg}_{instance_size}_optimizer_{optimizer._method}.png"
                        ...
                        # Plot convergence
                        try:
                            convergence_plot_fn = f"convergence_plot_{quant_alg}_optimizer_{optimizer._method}_layer_{N_LAYERS}_instance_size_{instance_size}.png"
                            ...
                            pylab.xlabel("Eval count")
                            pylab.ylabel("Energy")
                            pylab.title(f"Energy convergence for {instance_type_logging} -- p = {N_LAYERS}, optimizer: {optimizer.label}")
                            pylab.legend(loc="upper right")
